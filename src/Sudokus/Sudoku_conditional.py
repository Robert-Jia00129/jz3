...
class Sudoku:
    def __init__(self, sudoku_array):
        self._solver = Solver2SMT(benchmark_mode=True)
        self._grid = [[z3.Int(f'cell_{i}_{j}') for j in range(9)] for i in range(9)]
        self._distinct = z3.Bool('distinct')
        self._pbeq = z3.Bool('pbeq')
        self._solver.add_global_constraints(z3.Or(self._distinct, self._pbeq))
        self.load_numbers(sudoku_array)
...
def sudoku_conditional_constraints_test():
    ...
    result = sudoku._solver.check_conditional_constraints(sudoku._grid[0][0] == 1)
    ...
    models = sudoku._solver.get_condition_var_assignment_model()
...
if __name__ == '__main__':
    ...
    # TODO @sj cvc5 file path is not consistent